...
class LastMessageAggregator(MessageAggregator):
    ...
    def aggregate(self, unique_node_ids, messages):
        ...
        # return unique_messages, unique_timestamps
...
class MeanMessageAggregator(MessageAggregator):
    ...
    def aggregate(self, node_ids, messages):
        """aggregate the message in a batch"""
        unique_node_ids = np.unique(node_ids)
        unique_messages = []
        unique_timestamps = []

        for node_id in unique_node_ids:
            mess_total = list(list(zip(*messages[node_id]))[0])
            mess_node = torch.mean(torch.cat(mess_total,dim=1).to(self.device), dim=1)
            unique_messages.append(mess_node)
            unique_timestamps.append(messages[node_id][-1][1])
...
class TimeMessageAggregator(MessageAggregator):
    def __init__(self, device):
        super(TimeMessageAggregator, self).__init__(device)
        self.timeenc = TimeEncode(dimension=1)
        self.device = device

    def aggregate(self, node_ids, messages):
        """Keep weighted sum message for each node"""
        unique_node_ids = np.unique(node_ids)
        unique_messages = []
        unique_timestamps = []
        ...
        for node_id in unique_node_ids:
            if len(messages[node_id]) > 0:
                ...
                time_data = torch.stack([m[1] for m in messages[node_id]])
                weight_tm = self.timeenc(time_data.reshape(time_data.shape[0], -1))
                mess_data = torch.stack([m[0] for m in messages[node_id]])
                # weight_tm = torch.tensor(np.logspace(0,0.1**len(messages[node_id]),len(messages[node_id]),base=0.1,dtype='float32')).to(self.device)
                unique_messages.append(torch.sum(weight_tm.reshape(weight_tm.shape[0], -1) * mess_data, dim=0))
                unique_timestamps.append(messages[node_id][-1][1])
...
def get_message_aggregator(aggregator_type, device):
    ...
    elif aggregator_type == "addw":
        return TimeMessageAggregator(device=device)
    else:
